...
export function publishProgressReport(
  ...
  const tasks = db.prepare(`
    SELECT id, title, trade, status, progress, start_date, end_date, work_package_id
    FROM tasks
    WHERE project_id = ?
    ORDER BY wbs_code, title
  `).all(projectId);

  // 2. Snapshot certified progress claims
  const claims = db.prepare(`
    SELECT id, submission_no, work_package_id, claimed_percentage, certified_percentage, status, certified_amount
  ...
  const packages = db.prepare(`
    SELECT id, code, name, trade, progress_percentage, status
    FROM work_packages
    WHERE project_id = ?
  `).all(projectId);
