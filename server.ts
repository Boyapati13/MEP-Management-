...
function initDb() {
  db.exec(`
    ...
    /* Performance optimization: Create indexes on high-traffic foreign key and query columns
       to replace O(N) full table scans with O(log N) index lookups. */
    ...
    CREATE INDEX IF NOT EXISTS idx_plan_task_checklist_task_id ON plan_task_checklist(task_id);
